...
describe('host binding sanitization', () => {
  ...
  it('should reject a foreign-selector codebase host binding on a concrete object host', async () => {
    @Component({
      selector: 'object',
      template: '',
    })
    class DynamicObjectHostComponent {}

    // The declaring selector is not an element that has a `codebase` sink, so before the fix no
    // sanitizer was generated for this binding at all, even though the directive can be applied
    // to a concrete `<object>` host where `codebase` is a resource URL sink.
    @Directive({
      selector: 'safe-codebase-carrier',
      host: {'[attr.codebase]': 'codebase'},
    })
    class SafeCodebaseCarrierDirective {
      @Input() codebase = '';
    }

    @Component({
      template: '',
      imports: [DynamicObjectHostComponent],
    })
    class App {
      private viewContainerRef = inject(ViewContainerRef);
      private environmentInjector = inject(EnvironmentInjector);
      ...
      constructor() {
        this.viewContainerRef.createComponent(DynamicObjectHostComponent, {
          environmentInjector: this.environmentInjector,
          directives: [
            {
              type: SafeCodebaseCarrierDirective,
              bindings: [inputBinding('codebase', () => HOST_BINDING_URL)],
            },
          ],
        });
      }
  ...
  it('should sanitize a foreign-selector action host binding on a concrete form host', async () => {
    @Component({
      selector: 'form',
      template: '',
    })
    ...
    // As above: `action` is only a URL sink on `<form>`, so the declaring selector alone yields no
    // sanitizer, while the concrete host is a `<form>`.
    @Directive({
      selector: 'safe-action-carrier',
      host: {'[attr.action]': 'action'},
    })
    class SafeActionCarrierDirective {
      @Input() action = '';
    }
    ...
    class App {
      componentRef: ComponentRef<DynamicFormHostComponent>;

      private viewContainerRef = inject(ViewContainerRef);
      private environmentInjector = inject(EnvironmentInjector);

      constructor() {
        this.componentRef = this.viewContainerRef.createComponent(DynamicFormHostComponent, {
          environmentInjector: this.environmentInjector,
          directives: [
            {
              type: SafeActionCarrierDirective,
              bindings: [inputBinding('action', () => HOST_BINDING_UNSAFE_URL)],
            },
          ],
        });
      }
    ...
    const form = fixture.componentInstance.componentRef.location.nativeElement as HTMLFormElement;
    expect(form.getAttribute('action')).toBe(`unsafe:${HOST_BINDING_UNSAFE_URL}`);
  });
});
